...
class BaseConcurrencyTests : public PjRtGpuStreamExecutorConcurrencyTestBase {
 protected:
  ...
  absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> CompileStableHlo(std::string_view stablehlo_string, const DeviceMesh& mesh, DebugOptions* debug_options = nullptr);
  ...
  static std::vector<absl::Span<const LiteralSlice>> MakeInnerSpan(
