...
TEST(XlaCompilationTest, ExecuteOnMultpleStreamsSlice) {
  ...
  auto test_fun = [](int round_number) {
    ...
    // auto ASlice = A;
    auto ASlice = Slice(A, {0, 0}, {2024, 2024}, {1, 1});
    // auto BSlice = B;
    auto BSlice = Slice(B, {0, 0}, {2024, 2024}, {1, 1});

    XlaOp a_dot_b = Dot(ASlice, BSlice);

    XlaOp heavy = a_dot_b;
    for (int i = 0; i < 1; ++i) {
      heavy = Dot(heavy, Slice(A, {2024, 2024}, {4048, 4048}, {1, 1}));
      heavy = heavy - a_dot_b;
    }
    ...
    ASSERT_NEAR(tuple[0].Get<float>({0, 0}), 4132988.25, 1e-7);
  };
